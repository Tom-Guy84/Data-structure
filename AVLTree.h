...
namespace wet1_dast
{
    ...
    template<class T>
    void AVLTree<T>::Node::Tree_Creator_AUX(int *size_of_tree, int height_of_tree,int* remainder)
    {
    ...
    template<class T>
    int AVLTree<T>::Node::GetHeight(Node* node) {
        if(!node)
            return NOT_EXIST;
        return node->height;
    }
    ...
    template<class T>
    void AVLTree<T>::inorderHelper(T* values[] , int* index, Node *ver, int max_size)
    {
